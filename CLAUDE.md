...
| `dart_llm_batch.py` | **DART 사업보고서 LLM 배치 추출** (20개 변수, Claude Haiku 4.5, 발췌 v4.2) |
| `check_titles.py` | **XML heading 탐색** (DBO 위치 → TITLE/heading 역추적) |
| `title_check_result.csv` | heading 탐색 결과 (3,236사, HEADING 2,142 / TITLE 65) |
| `llm_extract_{2020~2024}.csv` | **v4.3 추출 결과** (5개년, 20변수, 연도당 ~1,100 OK) |
| `llm_extract_2024_v1.csv` | v1 추출 결과 (한글컬럼, 구버전) |
| `validate_xbrl_llm.py` | XBRL vs LLM 교차검증 스크립트 |
| `re_extract_and_llm.py` | 키워드 미포함 건 재발췌+LLM 재추출 |
| `re_extract_fstype.py` | 별도→연결 재발췌 (LLM 미호출) |
